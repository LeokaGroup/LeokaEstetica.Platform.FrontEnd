...
export class DetailProjectComponent implements OnInit {
  ...
  public readonly projectVacanciesColumns$ =
    this._projectService.projectVacanciesColumns$;
  public readonly availableAttachVacancies$ =
    this._projectService.availableAttachVacancies$;
  public readonly availableInviteVacancies$ =
    this._projectService.availableInviteVacancies$;
  ...
  public readonly availableVacansiesResponse$ =
    this._projectService.availableVacansiesResponse$;
  ...
  private async getEditProjectAsync(projectId: number, mode: string) {
    (await this._projectService.getProjectAsync(projectId, mode)).subscribe(
      (response: any) => {
        ...
        this.isVisibleActionProjectButtons =
          response.isVisibleActionProjectButtons;
        this.isVisibleActionLeaveProjectTeam =
          response.isVisibleActionLeaveProjectTeam;
        this.isVisibleActionAddProjectArchive =
          response.isVisibleActionAddProjectArchive;
        this.isShowRemarks =
          this.selectedProject$.value.projectRemarks.length > 0;
      }
    );
  }
  ...
  public async onUpdateProjectAsync() {
    ...
    (await this._projectService.updateProjectAsync(model)).subscribe(
      async (_) => {
        // TODO: setVisibleProjectAsync убрать после того как поправят
        // на бэке установку isPublic в updateProjectAsync
        await firstValueFrom(
          await this._projectService.setVisibleProjectAsync(
            this.projectId,
            model.isPublic
          )
        );
        ...
        this._router.navigateByUrl(
          `projects/project?projectId=${this.projectId}&mode=view`
        );
      }
    );
  }
  ...
  private async getProjectVacanciesColumnNamesAsync() {
    (
      ...
      console.log(
        'Столбцы таблицы вакансий проектов пользователя: ',
        this.projectVacanciesColumns$.value
      );
    });
  }
  ...
  private async getAvailableAttachVacanciesAsync() {
    (
      await this._projectService.getAvailableAttachVacanciesAsync(
        this.projectId
      )
    ).subscribe((_) => {
      console.log(
        'Доступные к привязке вакансии: ',
        this.availableAttachVacancies$.value
      );
      this.availableAttachVacancies =
        this.availableAttachVacancies$.value.projectVacancies;
    });
  }
  ...
  private async getAvailableInviteVacanciesAsync() {
    (
      await this._projectService.getAvailableInviteVacanciesAsync(
        this.projectId
      )
    ).subscribe((_) => {
      console.log(
        'Доступные к приглашению в проект вакансии: ',
        this.availableInviteVacancies$.value
      );
      this.availableInviteVacancies =
        this.availableInviteVacancies$.value.projectVacancies;
    });
  }
  ...
  public async onShowVacancyModal(vacancyId: number, isEdit: boolean) {
    ...
    (await this._vacancyService.getVacancyByIdAsync(vacancyId, mode)).subscribe(
      async (_) => {
  ...
  public async onUpdateVacancyAsync() {
    ...
    (await this._vacancyService.updateVacancyAsync(model)).subscribe(
      (response: any) => {
   ...
   */
  public async onShowProjectResponseWithVacancyModal(
    isResponseVacancy: boolean
  ) {
    this.isResponseVacancy = isResponseVacancy;

    (
      await this._projectService.availableVacanciesProjectResponseAsync(
        this.projectId
      )
    ).subscribe((response: any) => {
  ...
  public async onProjectResponseAsync() {
    ...
    (await this._projectService.writeProjectResponseAsync(model)).subscribe(
      (response: any) => {
        if (response.errors !== null && response.errors.length > 0) {
          response.errors.forEach((item: any) => {
            this._messageService.add({
  ...
  public async onWriteOwnerDialogAsync() {
    ...
    (await this._messagesService.writeOwnerDialogAsync(dialogInput)).subscribe(
      async (_) => {
  ...
  public async onCreateProjectCommentAsync() {
    ...
    (
      await this._projectService.createProjectCommentAsync(
  ...
  private async getProjectTeamColumnsNamesAsync() {
    (await this._projectService.getProjectTeamColumnsNamesAsync()).subscribe(
      async (response: any) => {
  ...
  private async getProjectTeamAsync() {
    (await this._projectService.getProjectTeamAsync(this.projectId)).subscribe(
      async (response: any) => {
  ...
  public async onSearchInviteProjectMembersAsync(event: any) {
    (
      await this._searchProjectService.searchInviteProjectMembersAsync(
        event.query
      )
    ).subscribe(async (response: any) => {
  ...
  public async onSendInviteProjectTeamAsync() {
    const inviteProjectTeamMemberInput: InviteProjectTeamMemberInput = {
      ...
      VacancyId: !this.isVacancyInvite
        ? this.selectedInviteVacancy.vacancyId
        : null,
      InviteType: 'Email',
    };

    (
      await this._projectService.sendInviteProjectTeamAsync(
        inviteProjectTeamMemberInput
      )
    ).subscribe(async (response: any) => {
      ...
      this._messageService.add({
        severity: 'success',
        summary: 'Все хорошо',
        detail: response.successMessage,
      });
    });
  ...
  public async onDeleteProjectAsync() {
    (await this._projectService.deleteProjectsAsync(this.projectId)).subscribe(
      async (response: any) => {
  ...
  public onBeforeDeleteProjectVacancy(
    vacancyId: number,
    vacancyNameForDelete: any
  ) {
    this.vacancyId = vacancyId;
  ...
  public async onDeleteVacancyInProjectAsync() {
    (
      await this._projectService.deleteVacancyInProjectAsync(
        this.projectId,
        this.vacancyId
      )
    ).subscribe(async (_) => {
  ...
  public async onDeleteProjectTeamAsync() {
    (
      await this._projectService.deleteProjectTeamAsync(
        this.projectId,
        this.userId
      )
    ).subscribe(async (_) => {
  ...
  public async onSetProjectTeamMemberRoleAsync(userId: number, role: string) {
    (
      await this._projectService.setProjectTeamMemberRoleAsync(
        userId,
        role,
        this.projectId
      )
